...
export const STATUS_CONFIG: Record<AppStatus | 'all', { label: string; color: string; bgColor: string; icon: React.ReactNode }> = {
  all: { label: 'Tout', color: 'text-text-primary', bgColor: 'bg-white', icon: <Briefcase size={16} /> },
  ...
  ghosted: { label: 'Silence', color: 'text-text-secondary', bgColor: 'bg-gray-100', icon: <Ghost size={16} /> },
};

export const FOLLOW_UP_CONFIG: Record<FollowUpStatus, { label: string; color: string; bgColor: string; icon: React.ReactNode }> = {
  none: { label: 'Aucune', color: 'text-text-secondary', bgColor: 'bg-gray-50', icon: <Clock size={14} /> },
